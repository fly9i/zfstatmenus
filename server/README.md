...
| 方法 | 路径 | 认证 | 说明 |
| --- | --- | --- | --- |
| `GET` | `/` 或 `/dashboard` | 否 | 同步面板页面（数据接口仍需 Token） |
| `GET` | `/v1/health` | 否 | 健康检查与服务器时间 |
| `GET` | `/v1/me` | 是 | 当前用户及设备列表 |
| `POST` | `/v1/sync` | 是 | 幂等写入设备每日完整快照 |
| `GET` | `/v1/snapshot` | 是 | 获取当前用户的远程设备数据 |
| `GET` | `/v1/stats` | 是 | 按日/模型/来源/设备汇总，费用按「用户定价 > 内置定价目录」计算，模型行含定价来源 |
| `GET` | `/v1/pricing` | 是 | 列出当前用户的模型定价 |
| `PUT` | `/v1/pricing` | 是 | 新增或更新模型定价（provider/model 大小写无关） |
| `DELETE` | `/v1/pricing` | 是 | 删除模型定价（`?provider=&model=`） |

## 同步面板

部署后访问 Worker 根路径即可打开同步面板。页面本身无需认证，首次使用在页面上填入访问 Token（仅保存在浏览器 localStorage），即可查看与客户端类似的汇总：今日/近 7 天/近 30 天 Token 与费用、模型用量、每日明细、来源分布与设备。

- 费用按「用户自定义定价（`model_pricing` 表）> 内置定价目录（`src/pricing.ts`，与客户端 `ModelPricingCatalog` 同步）」的优先级计算；模型行会标注定价来源（自定义/内置/未定价），可在页面「定价」中为任意使用过的模型编辑单价（输入/缓存读取/缓存写入/输出推理，单位均为每百万 Token），支持 USD 与 CNY。
- 定价按用户隔离，不同 Token 互不可见；删除自定义定价后自动回退到内置价格。
- 页面会自动获取 USD/CNY 汇率用于双币种换算，也可手动修改。
